document.addEventListener('nexus:ready',()=>{
  ...
  N.renderSettings=async()=>{N.body.replaceChildren();const s=N.settings;const box=document.createElement('div');box.className='nexus-settings';
    ...
    const edge=document.createElement('select');['left','right'].forEach(v=>edge.add(new Option(v,v,v===s.edge,v===s.edge)));edge.onchange=()=>save('edge',edge.value);appearance.append(row('Side',edge));
    const floating=input('checkbox',s.railStyle==='floating');floating.onchange=()=>save('railStyle',floating.checked?'floating':'bar');appearance.append(row('Floating rails',floating,'Separators split icons into separate rails'));
    ...
    const aut=input('checkbox',s.autoHide);aut.onchange=()=>save('autoHide',aut.checked);behavior.append(row('Auto-hide rail',aut,'Open panels stay open until you click outside or close them'));
    const delay=input('range',s.autoHideDelay);delay.min=100;delay.max=2500;delay.oninput=()=>save('autoHideDelay',+delay.value);behavior.append(row('Rail hide delay',delay));
    ...
    const icons=section('Icons & rails');
    const list=document.createElement('div');list.className='nexus-icon-visibility';for(const item of [...N.FEATURES,...N.sites]){const lab=document.createElement('label');const cb=input('checkbox',!(s.hiddenIcons||[]).includes(item.id));cb.onchange=async()=>{const set=new Set(s.hiddenIcons||[]);cb.checked?set.delete(item.id):set.add(item.id);s.hiddenIcons=[...set];await N.saveSettings();N.renderRail();};lab.append(cb,document.createTextNode(item.name));list.append(lab)}icons.append(row('Visible icons',list,'Settings, Edit and Session Hide always remain available'));
    const railActions=document.createElement('div');railActions.className='nexus-inline-actions';const edit=document.createElement('button');edit.textContent=N.editMode?'Finish edit mode':'Enter edit mode';edit.onclick=()=>{N.editMode=!N.editMode;N.apply();N.renderRail();N.renderSettings();};const addSep=document.createElement('button');addSep.textContent='Add separator';addSep.onclick=()=>N.addSeparator();const reset=document.createElement('button');reset.textContent='Reset rail';reset.onclick=async()=>{s.railLayout=[];await N.saveSettings();N.renderRail();N.renderSettings();};railActions.append(edit,addSep,reset);icons.append(row('Rail layout',railActions,'Drag icons and separators directly on the rail while editing'));box.append(icons);
