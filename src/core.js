(() => {
  ...
  function msg(m) {
    return new Promise(r => {
      try {
        chrome.runtime.sendMessage(m, x => r(chrome.runtime.lastError ? { ok: false, error: chrome.runtime.lastError.message } : (x || { ok: true })));
      } catch (e) {
  ...
  function wildcard(pattern, value) {
    const e = pattern.replace(/[.+^${}()|[\]\\]/g, '\\$&').replace(/\*/g, '.*');
  ...
  function validItemIds(features, sites) {
    return new Set([...features.map(x => x.id), ...sites.map(x => x.id)]);
  }

  function normalizeLayout(settings, sites) {
    const valid = validItemIds(FEATURES, sites);
    ...
    for (const t of layout) {
      if (String(t).startsWith('sep:') && String(compact.at(-1) || '').startsWith('sep:')) continue;
      compact.push(t);
    }
    while (compact.length && String(compact.at(-1)).startsWith('sep:')) compact.pop();
    settings.railLayout = compact;
    return compact;
  }
  ...
  async function boot(force = false) {
    ...
    try {
      ...
      if (!force && !allowedByRules(settings)) {
        booting = false;
        return;
      }
      ...
      const N = window.NexusSidebar = {
        settings, sites, games: got.nexusGames || {}, FEATURES, storeGet, storeSet, msg, quotes,
        active: null, sessions: new Map(), cleanup: [], editMode: false
      };
      ...
      N.root = root;
      N.rail = root.querySelector('#nexus-rail');
      N.panel = root.querySelector('#nexus-panel');
      N.body = root.querySelector('#nexus-body');
      N.icons = root.querySelector('#nexus-icons');
      N.sessionsHost = root.querySelector('#nexus-sessions');
      const hotzone = root.querySelector('#nexus-hotzone');

      const style = document.createElement('style');
      style.id = 'nexus-custom-css';
      ...
      N.iconNode = (item, custom = false) => {
        if (custom || item.type === 'web') {
          ...
          i.onerror = () => {
            ...
            s.textContent = custom ? '◫' : item.icon;
            i.replaceWith(s);
          };
          return i;
        }
      ...
      N.setHeader = (item, custom = false) => {
        const h = root.querySelector('#nexus-head-icon');
        h.replaceChildren(N.iconNode(item, custom));
      ...
      N.show = () => {
        cancelHide();
        sessionStorage.removeItem('nexus-session-hidden');
        root.classList.remove('session-hidden');
        root.classList.add('rail-visible');
      };

      N.hideRail = () => {
        cancelHide();
        if (!N.panel.classList.contains('open')) root.classList.remove('rail-visible');
      };
      ...
      N.scheduleAutoHide = (delay = N.settings.autoHideDelay) => {
        ...
        hideTimer = setTimeout(() => {
          hideTimer = 0;
          if (N.panel.classList.contains('open') || N.editMode) return;
          if (N.rail.matches(':hover') || hotzone.matches(':hover') || N.panel.matches(':hover')) return;
          N.hideRail();
      ...
      N.hideSession = () => {
        cancelHide();
        sessionStorage.setItem('nexus-session-hidden', '1');
        N.panel.classList.remove('open');
        root.classList.add('session-hidden');
      };

      N.runCleanup = () => {
        for (const fn of N.cleanup.splice(0)) {
          try { fn(); } catch {}
        }
      };
      ...
      N.apply = () => {
        const s = N.settings;
        const wasVisible = root.classList.contains('rail-visible');
        ...
        root.style.setProperty('--nexus-icon', s.iconSize + 'px');
        root.style.setProperty('--nexus-gap', s.iconGap + 'px');
        root.style.setProperty('--nexus-width', s.panelWidth + 'px');
        root.style.setProperty('--nexus-panel-gap', s.panelGap + 'px');
        root.style.setProperty('--nexus-radius', s.radius + 'px');
        root.style.setProperty('--nexus-reveal', s.revealWidth + 'px');
        root.style.setProperty('--nexus-accent', s.accent);
        style.textContent = s.customCSS || '';
        if (!s.autoHide) root.classList.add('rail-visible');
        else N.scheduleAutoHide();
      };
      ...
      const enter = () => N.show();
      const leave = () => N.scheduleAutoHide();
      hotzone.addEventListener('pointerenter', enter);
      hotzone.addEventListener('pointerleave', leave);
      N.rail.addEventListener('pointerenter', enter);
      N.rail.addEventListener('pointerleave', leave);
      N.panel.addEventListener('pointerenter', cancelHide);
      N.panel.addEventListener('pointerleave', leave);
      ...
      root.querySelector('#nexus-edit').onclick = () => {
      ...
      const outside = e => {
        if (!N.panel.classList.contains('open')) return;
        if (N.panel.contains(e.target) || N.rail.contains(e.target)) return;
        N.closePanelSoft();
      };
      document.addEventListener('pointerdown', outside, true);
      N.cleanupRoot = () => document.removeEventListener('pointerdown', outside, true);
  ...
  chrome.runtime.onMessage.addListener(m => {
    ...
    if (window.NexusSidebar) {
      window.NexusSidebar.show();
      window.NexusSidebar.scheduleAutoHide?.(1800);
      return;
    }
    boot(true).then(() => {
      window.NexusSidebar?.show();
