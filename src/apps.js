document.addEventListener('nexus:ready', () => {
  ...
  function moveToken(source, target) {
    ...
    const from = a.indexOf(source);
    const to = a.indexOf(target);
  ...
  function draggable(el, token) {
    ...
    el.addEventListener('drop', e => {
      e.preventDefault();
      const src = e.dataTransfer.getData('text/nexus-token');
      if (src) moveToken(src, token);
    });
  }

  function iconButton(item, custom = false) {
    const b = document.createElement('button');
    b.className = 'nexus-icon' + (N.active?.id === item.id ? ' active' : '');
    ...
    b.onclick = e => {
      ...
      N.activate(item, custom);
    };
    draggable(b, item.id);
    return b;
  }

  function sepHandle(token) {
    const d = document.createElement('button');
    d.className = 'nexus-separator-handle';
    d.title = 'Drag separator · double-click to remove';
    d.textContent = '⋯';
    draggable(d, token);
    d.ondblclick = async () => {
      N.settings.railLayout = N.settings.railLayout.filter(x => x !== token);
      await saveLayout();
      N.renderRail();
    };
    return d;
  ...
  function makeGroup() {
    ...
    g.className = 'nexus-rail-group';
    return g;
  }

  N.addSeparator = async () => {
    const a = N.normalizeLayout();
    if (!a.length) return;
    const token = 'sep:' + crypto.randomUUID();
    let at = a.indexOf(N.active?.id);
    if (at < 0) at = Math.max(0, Math.floor(a.length / 2) - 1);
    a.splice(at + 1, 0, token);
  ...
  N.renderRail = () => {
    ...
    const layout = N.normalizeLayout();
    let group = makeGroup();
    N.icons.append(group);
    for (const token of layout) {
      if (String(token).startsWith('sep:')) {
        if (N.editMode) N.icons.append(sepHandle(token));
        group = makeGroup();
        N.icons.append(group);
        continue;
      }
      const item = getItem(token);
      if (!item || hidden(token)) continue;
      group.append(iconButton(item, String(item.id).startsWith('site-')));
    }
    [...N.icons.querySelectorAll('.nexus-rail-group')].forEach(g => {
      if (!g.children.length && !N.editMode) g.remove();
    });
    N.root.querySelector('#nexus-add-separator').hidden = !N.editMode;
    N.root.querySelector('#nexus-edit').textContent = N.editMode ? '✓' : '✎';
    N.root.querySelector('#nexus-edit').title = N.editMode ? 'Finish editing' : 'Edit rail';
  };
  ...
  N.activate = async (item, custom = false) => {
    ...
    else if (item.id === 'calculator') renderCalc();
  ...
  function renderLaunchpad() {
    ...
    time.className = 'nexus-card';
    const weather = document.createElement('div');
    weather.className = 'nexus-card';
    ...
    N.cleanup.push(() => clearInterval(t));
    ...
    [
      ...
      ['◷', 'History', () => N.activate(N.FEATURES[1])],
      ['＋', 'Calculator', () => N.activate(N.FEATURES[3])],
      ['🏎️', 'F1 race', () => N.activate(N.FEATURES[4])]
    ].forEach(([i, n, f]) => {
      const b = document.createElement('button');
      b.innerHTML = `<b>${i}</b><small>${n}</small>`;
      b.onclick = f;
      qa.append(b);
    });
    ...
    [...N.FEATURES.filter(x => x.id !== 'launchpad' && !hidden(x.id)), ...N.sites.filter(x => !hidden(x.id))].forEach(x => {
      const b = document.createElement('button');
      b.append(N.iconNode(x, String(x.id).startsWith('site-')));
      const s = document.createElement('small');
      s.textContent = x.name;
      b.append(s);
      b.onclick = () => N.activate(x, String(x.id).startsWith('site-'));
      apps.append(b);
    });
  ...
  async function renderHistory() {
    ...
    input.className = 'nexus-wide';
    input.placeholder = 'Search history';
    const list = document.createElement('div');
    list.className = 'nexus-list';
    N.body.append(input, list);
    const load = async () => {
      const r = await N.msg({ type: 'nexus:history', query: input.value });
  ...
  function renderCalc() {
    N.body.innerHTML = '<div class="nexus-calc"><input placeholder="2 * (3 + 4)"><button>=</button><output>0</output></div>';
    const i = N.body.querySelector('input');
    const o = N.body.querySelector('output');
    N.body.querySelector('button').onclick = () => {
      try {
        const s = i.value;
        if (!/^[0-9+\-*/().%\s]+$/.test(s)) throw 0;
        o.textContent = String(Function('return (' + s + ')')());
      } catch {
        o.textContent = 'Error';
      }
    };
  ...
  // Important: render the rail, but do not auto-open Launchpad on every webpage.
  // The old behavior made each navigation look like Nexus had replaced the page.
  N.renderRail();
