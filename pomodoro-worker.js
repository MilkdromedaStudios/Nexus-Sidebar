...
const DEFAULT_PREFS = { shortBreakMinutes:5, longBreakMinutes:15, longBreakEvery:4, silent:false, sound:'notification', autoSuggestBreak:true };
...
async function getPomodoro() {
  ...
  if (state.running && state.endAt) {
    ...
    if (state.remainingSec <= 0) { state.running=false; state.endAt=0; state.remainingSec=0; await pSet({ [POMODORO_KEY]: state }); }
  }
  return state;
}
...
async function playFocusSound(sound){
  if(!['soft','bell'].includes(sound))return;
  try{const [tab]=await chrome.tabs.query({active:true,lastFocusedWindow:true});if(tab?.id)chrome.tabs.sendMessage(tab.id,{type:'nexus:pomodoro-sound',sound},()=>void chrome.runtime.lastError);}catch{}
}
async function finishPomodoro() {
  ...
  history.unshift({id:crypto.randomUUID(),mode:finishedMode,durationSec,label,finishedAt:Date.now()});const trimmed=history.slice(0,500);
  state.running=false;state.endAt=0;state.remainingSec=0;
  if(finishedMode==='focus'&&prefs.autoSuggestBreak){const today=new Date().toDateString(),count=trimmed.filter(x=>x.mode==='focus'&&new Date(x.finishedAt).toDateString()===today).length,long=count>0&&count%Math.max(1,prefs.longBreakEvery||4)===0;state.suggestedBreakMinutes=long?(prefs.longBreakMinutes||15):(prefs.shortBreakMinutes||5);}
  ...
  if(!data.nexusDoNotDisturb&&!prefs.silent){await playFocusSound(prefs.sound);try{await chrome.notifications.create('nexus-pomodoro-'+Date.now(),{type:'basic',iconUrl:chrome.runtime.getURL('icons/icon.png'),title,message,priority:2});}catch{}}
}
