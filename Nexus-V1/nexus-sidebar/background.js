...
function setupAlarms() {
  ...
  chrome.alarms.create('nexus-media-monitor', { periodInMinutes: 1 });
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
  switch (alarm.name) {
    ...
    case 'nexus-check-habits':
      ...
      break;
    case 'nexus-media-monitor':
      await checkMediaSessions();
      break;
...
async function checkMediaSessions() {
  const { nexusSettings, nexusMediaSessions } = await chrome.storage.local.get(['nexusSettings', 'nexusMediaSessions']);
  if (!nexusSettings?.mediaActivityMonitor) return;

  const previous = Array.isArray(nexusMediaSessions) ? nexusMediaSessions : [];
  const prevById = new Map(previous.map(s => [s.id, s]));
  const tabs = await chrome.tabs.query({});
  const sessions = tabs
    .filter(t => t.id && t.audible && t.url && /^https?:/.test(t.url))
    .map(t => ({
      id: t.id,
      title: t.title || 'Media playing',
      url: t.url,
      audible: true,
      startedAt: prevById.get(t.id)?.startedAt || Date.now(),
      lastSeen: Date.now()
    }));

  const newSessions = sessions.filter(s => !prevById.has(s.id));
  if (nexusSettings.notificationsEnabled && newSessions.length) {
    chrome.notifications.create({
      type: 'basic',
      iconUrl: 'icons/icon128.svg',
      title: '🔊 Media activity',
      message: newSessions[0].title.slice(0, 100)
    });
  }
